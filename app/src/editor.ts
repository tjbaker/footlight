...
export function mountEditor(root: HTMLElement): void {
   ...
   * while proposals await Accept / Step / Discard. Set via `setGhosts` (the
   * timeline view keeps its own copy for the track's ghost DOM); nothing here
   * mutates editor state — that's the commit's job.
   */
